...
const video_slider_videos = document.querySelectorAll('.video-slider-inner iframe');
...
video_slider_next.addEventListener('click', function(){
  ...
  for (let i = 0; i< video_slider_controls.length; i++){
    ...
    if(video_slider_controls[i].classList.contains('slider-controls-item-active')){
      ...
      video_slider.style.transition = 'all 0.3s ease-in-out';
      video_slider.style.transform = `translateX(-40.69%)`
      video_slider.addEventListener('transitionend', function onTransitionEnd() {
      video_slider.removeEventListener('transitionend', onTransitionEnd);
      video_slider.append(video_slider_videos[i])
      video_slider.style.transition = 'none';
      video_slider.style.transform = 'translateX(-20.345%)';
      })
      if(i==4){
        ...
        video.poster = `./assets/img/poster0.jpg`
      ...
      video.poster = `./assets/img/poster${i+1}.jpg`
...
video_slider_before.addEventListener('click', function(){
  ...
  for (let i = 4; i>= 0; i--){
    if(video_slider_controls[i].classList.contains('slider-controls-item-active')){
      video_slider.style.transition = 'all 0.3s ease-in-out';
      video_slider.style.transform = `translateX(0)`
      video_slider.addEventListener('transitionend', function onTransitionEnd() {
      video_slider.removeEventListener('transitionend', onTransitionEnd);
      video_slider.prepend(video_slider_videos[(i+4)%5])
      video_slider.style.transition = 'none';
      video_slider.style.transform = 'translateX(-20.345%)';
      })
      if(i==0){
        ...
        video.poster = `./assets/img/poster4.jpg`
  ...
  video_slider_controls[i].addEventListener('click', function() {
    for (let j = 0; j< video_slider_controls.length; j++){   
      ...
      if(video_slider_controls[j].classList.contains('slider-controls-item-active')){
        video_slider.style.transition = 'all 0.3s ease-in-out';
        if (j<i){
          video_slider.style.transform = `translateX(${-20.345 * (i+1-j)}%)`
        }
        else{
          video_slider.style.transform = `translateX(${20.345 * (j-i-1)}%)`
        }
        video_slider.addEventListener('transitionend', function onTransitionEnd() {
          video_slider.removeEventListener('transitionend', onTransitionEnd);
          for(let k = 0; k<document.querySelectorAll('.video-slider-inner iframe').length; k++){
            if(document.querySelectorAll('.video-slider-inner iframe')[k] == video_slider_videos[i]){
              let dynamic_video_slider = document.querySelectorAll('.video-slider-inner iframe')
              if(i>j){
                for(let l = 0; l<k;l++){
                  video_slider.append(dynamic_video_slider[l])
                }
              }
              else{
                for(let l = 4; l>=k; l--){
                  video_slider.prepend(dynamic_video_slider[l])
                }
              }
          ...
          for(let k = 0; k<video_slider_videos.length; k++){
        ...
        video.poster = `./assets/img/poster${i}.jpg`
...
window.addEventListener('resize', () =>{
  explore_slider_width = document.querySelector('.explore-slider').clientWidth
  image_after.style.width = explore_slider_width + 'px';
})
